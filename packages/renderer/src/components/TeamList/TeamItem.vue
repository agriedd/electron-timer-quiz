...
<template>
  <div class="relative">
    <div
      class="p-4 text-left flex rounded-lg cursor-pointer"
      :class="[{'bg-teal-50': firstPosition, border: !firstPosition}]"
      @click="show_options = !show_options"
    >
      <div class="fill-teal-500 pr-3 w-20">
        <div
          class="flex justify-center items-center h-full relative"
          v-if="firstPosition"
        >
          <div class="absolute fill-teal-600">
            <svg
              ...
              width="48"
              height="48"
              class="bi bi-award-fill drop-shadow-lg"
              viewBox="0 0 16 16"
            >
              <path
    ...
    <div
      class="py-3 text-left flex"
      v-if="show_options"
    >
      <div class="flex">
        <button
          class="p-3 bg-indigo-50 rounded-lg mr-3"
          @click="resetPoint"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="flex-1">
        <div class="flex">
          ...
          <input
            type="number"
            class="px-3 py-2 shrink w-24 mr-3 rounded-lg border"
            step="5"
            v-model="input_point"
          />
          <button
            class="p-3 bg-indigo-50 rounded-lg"
            @click="increasePoint"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
